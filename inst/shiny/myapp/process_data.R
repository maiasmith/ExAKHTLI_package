...
add_initial_metrics <- function(df, start, end, time_elapsed) {
  ...
  df$AUC_trapezoidal_integration <- apply(df[start:end], 1, function(x) pracma::trapz(time_elapsed, x)) #correct AUC? boundary correction for trapezoidal integration? ?trapz
...
get_confidence_intervals_neg_controls <- function(df, phenotypic_Marker, first_timepoint_index, last_timepoint_index) {
  ...
  confidence_intervals <- apply(negControls, 2, function(x) Rmisc::CI(x, ci=0.999)) # 99.9% confidence intervals
