...
parse_compound_key <- function(key_filename) {
    key_wb <- XLConnect::loadWorkbook(key_filename$datapath)
    key_data_raw <- XLConnect::readWorksheet(key_wb, sheet = getSheets(key_wb))
...
parse_compound_info <- function(compound_info_filename) {
  compound_wb <- XLConnect::loadWorkbook(compound_info_filename$datapath)
  input_compound_info <- XLConnect::readWorksheet(compound_wb, sheet = getSheets(compound_wb))
...
parse_incucyte_data <- function(incucyte_data_filename, key_data) {
  incucyte_wb <- XLConnect::loadWorkbook(incucyte_data_filename$datapath)
  incucyte_data_raw <- XLConnect::readWorksheet(incucyte_wb, sheet = getSheets(incucyte_wb))
